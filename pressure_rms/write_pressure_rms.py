...
def write_pressure_rms(output_dir, initial_state, cenH_size, cenH_init_idx, N,
                       t_total, noise, alpha_1, alpha_2, beta, seed,
                       dist_vecs_to_com, pressure_size):

    # Make filename
    write_name = output_dir + 'statistics/pressure_RMS_'
    write_name += (
        f'init_state={initial_state}_cenH={cenH_size}_'
        + f'cenH_init_idx={cenH_init_idx}_N={N}_t_total={t_total}_'
        + f'noise={noise:.4f}_alpha_1={alpha_1:.5f}_alpha_2={alpha_2:.5f}_'
    ...
    rms = torch.mean(
        torch.square(torch.norm(dist_vecs_to_com[int(shape_0/2):], dim=2)),
        dim=1)
