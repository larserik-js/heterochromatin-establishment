...
def curied_run(constant):
    return run.run(N, l0, noise, dt, t_total, U_two_interaction_weight, U_pressure_weight, constant*alpha_1,
                   ...
                   allow_state_change=allow_state_change, cenH=cenH, write_cenH_data=write_cenH_data,
                   barriers=barriers, verbose=True)
...
if __name__ == '__main__':
    ...
    if multi:
        ...
        res = list(pool.map(curied_run, constant_list, chunksize=1))
        ...
        run.run(N, l0, noise, dt, t_total, U_two_interaction_weight, U_pressure_weight, alpha_1, alpha_2, beta,
                stats_t_interval, seed, test_mode, animate, allow_state_change, cenH, write_cenH_data, barriers,
                verbose=True)
